...
const AR = (() => {
  ...
  // ---- MODO AR: dos formas de ver el MISMO modelo -----------------------
  //   "camera"  = sin marcador. El modelo flota centrado frente a la cámara
  //               y se coloca/quita con el botón. Funciona en todo móvil.
  //   "marker"  = con marcador Hiro impreso. El modelo aparece anclado sobre
  //               el marcador y lo sigue (AR.js / THREEx). Requiere imprimir
  //               o mostrar el marcador y apuntarle la cámara.
  // En AMBOS modos, placedRoot (que contiene el modelo y todos sus gestos)
  // es el mismo: solo cambia de quién cuelga —de la escena (camera) o de
  // markerRoot (marker)— así que el equipo se ve y se manipula idéntico.
  let arMode = "camera";          // modo activo
  let markerRoot = null;          // THREE.Group controlado por ArMarkerControls (modo marcador)
  let arToolkitSource = null;     // fuente de vídeo de AR.js (solo modo marcador)
  let arToolkitContext = null;    // contexto de tracking de AR.js (solo modo marcador)
  let markerControls = null;      // ArMarkerControls (solo modo marcador)
  let markerReady = false;        // el contexto de AR.js ya inicializó
  let markerVisible = false;      // el Hiro está a la vista ahora mismo
  let cameraProjBackup = null;    // matriz de proyección de perspectiva normal (modo cámara), para restaurar tras el modo marcador

  // Debounce de pérdida del marcador: oclusiones breves (mano, brillo,
  // motion blur) no deben hacer parpadear el modelo. Se muestra al instante
  // y se oculta solo si estuvo perdido más de este tiempo.
  ...
  function initARScene() {
    ...
    // RENDIMIENTO vs calidad: el pixelRatio multiplica el trabajo de la GPU
    // al cuadrado. 2.5 se veía nítido pero iba lento en gama media, así que
    // se limita a 1.75 (buen equilibrio nitidez/fluidez en el móvil). En
    // pantallas de baja densidad usa el valor real si es menor.
    ...
    // Sombras: activadas pero con mapa básico (más barato que PCFSoft) para
    // no penalizar el framerate en el móvil.
    ...
    // markerRoot: ancla del marcador Hiro (modo marcador). ArMarkerControls
    // escribe su matriz cada frame para que siga al marcador. En modo cámara
    // permanece vacío y oculto. placedRoot se reparenta dentro de él cuando
    // se activa el modo marcador (ver setMode).
    markerRoot = new THREE.Group();
    markerRoot.visible = false;
    scene.add(markerRoot);

    // placedRoot: contiene el modelo y su sombra. En modo CÁMARA cuelga de la
    // escena, centrado frente a la cámara (X=0, ligeramente abajo, a
    // PLACE_DEPTH). En modo MARCADOR cuelga de markerRoot y se resetea al
    // origen para asentarse sobre el marcador. Los gestos del alumno
    // modifican su rotación (orbitar), escala (pellizco) y posición X/Y.
  ...
  function initCamera(onReady) {
    ...
    const constraints = {
      ...
      video: {
        ...
        // 720p es más que suficiente de fondo y descarga al decodificador;
        // pedir 30fps ayuda a que el stream se sienta fluido.
  ...
  // -----------------------------------------------------------------------
  // §1b. ARRANQUE DEL MODO MARCADOR (AR.js / THREEx) — usa la librería
  // vendorizada en libs/arjs-threex.js. AR.js crea y gestiona su propio
  // <video> de cámara y calcula la pose del marcador Hiro cada frame,
  // escribiéndola en markerRoot. El modelo (placedRoot) cuelga de markerRoot
  // y así lo sigue. Requiere el marcador impreso o mostrado en otra pantalla.
  // -----------------------------------------------------------------------
  function initMarker(onReady) {
    if (typeof THREEx === "undefined") {
      showToast("No se pudo iniciar el modo marcador: falta la librería AR.js. Usa el modo Cámara.");
      stop();
      return;
    }
    ...
    arToolkitSource.init(function onSourceReady() {
      ...
      // que es lo que mantiene alineado el tracking con lo que se ve.
      const wrap = document.getElementById("arVideoWrap");
      ...
      // CLAVE: inicializar el contexto de AR.js sin depender SOLO del evento
      // "canplay". En muchos móviles el vídeo ya está listo (readyState alto)
      // cuando se añade el listener, así que "canplay" ya pasó y {once:true}
      // no vuelve a dispararlo → AR.js nunca arrancaba y no aparecía nada.
      // Ahora: si ya está listo, arranca de inmediato; si no, con canplay; y
      // como último respaldo, un timeout que fuerza el arranque.
      let contextStarted = false;
      const startContextOnce = () => {
        if (contextStarted) return;
        contextStarted = true;
        setupMarkerContext(onReady);
      };
      if (video && video.readyState >= 2) {
        startContextOnce();
      } else if (video) {
        video.addEventListener("canplay", startContextOnce, { once: true });
        video.addEventListener("loadeddata", startContextOnce, { once: true });
      }
      // Respaldo: si en 1.2 s no arrancó por eventos, forzarlo igual.
      setTimeout(startContextOnce, 1200);

      // Reajustes escalonados: AR.js necesita que el vídeo tenga dimensiones
      // reales antes de encajar el canvas de detección.
      setTimeout(onResizeMarker, 300);
      setTimeout(onResizeMarker, 800);
      setTimeout(onResizeMarker, 1600);
  ...
  function setupMarkerContext(onReady) {
    if (!arToolkitSource) return; // se salió/cambió de modo antes de arrancar
    try {
      ...
      arToolkitContext.init(() => {
        // AR.js impone su matriz de proyección calibrada. La cámara es la misma
        // que en modo cámara; guardamos la perspectiva normal para restaurarla
        // al salir/cambiar de modo.
        cameraProjBackup = cameraProjBackup || camera.projectionMatrix.clone();
        camera.projectionMatrix.copy(arToolkitContext.getProjectionMatrix());

        // changeMatrixMode:"modelViewMatrix" asume la cámara en el origen sin
        // rotación (AR.js escribe la pose relativa en markerRoot). Aseguramos
        // que así sea, o el modelo se coloca en un sitio equivocado.
        camera.position.set(0, 0, 0);
        ...
        markerControls = new THREEx.ArMarkerControls(arToolkitContext, markerRoot, {
          ...
          // Suavizado ligero: reduce el temblor sin exigir una detección tan
          // perfecta (un smoothing agresivo hacía que el marcador casi nunca
          // llegara a contar como "encontrado").
          smooth: true,
          smoothCount: 5,
        ...
        markerReady = true;
        onResizeMarker();
        showToast("Modo marcador listo · apunta al Hiro");
        if (onReady) onReady();
      });
    } catch (e) {
      showToast("No se pudo iniciar el tracking del marcador. Prueba el modo Cámara.");
    }
  }

  // El modo marcador usa la maquinaria de resize de AR.js (ajusta el vídeo y
  // el canvas de detección). El render/canvas 3D se ajusta aparte igual que
  // en modo cámara.
  function onResizeMarker() {
    if (arToolkitSource) {
  ...
  // Detiene y limpia toda la maquinaria de AR.js (al salir o cambiar a modo
  // cámara). Restaura la proyección de perspectiva normal de la cámara.
  function teardownMarker() {
    ...
    if (cameraProjBackup && camera) {
      camera.projectionMatrix.copy(cameraProjBackup);
    }
  }
  ...
  // -----------------------------------------------------------------------
  // -----------------------------------------------------------------------
  // §2b. CAMBIO DE MODO (cámara ↔ marcador) — reparenta placedRoot (el
  // modelo) para que en modo cámara cuelgue de la escena centrado frente a
  // la cámara, y en modo marcador cuelgue de markerRoot para seguir al Hiro.
  // El modelo y sus gestos son idénticos: solo cambia su padre y su pose.
  // Se llama con la sesión ya activa (durante start o desde el interruptor).
  // -----------------------------------------------------------------------
  function applyModeToPlacedRoot() {
    if (!placedRoot) return;
    if (arMode === "marker") {
      // Cuelga de markerRoot y se asienta en su origen (sobre el marcador).
      if (placedRoot.parent !== markerRoot) markerRoot.add(placedRoot);
  ...
  // Ajusta qué controles del overlay tienen sentido en cada modo:
  //  - "Colocar/Quitar" y "Centrar" solo aplican en modo cámara.
  //  - En modo marcador aparece una pista de "apunta al marcador".
  function syncModeUI() {
    ...
    const inMarker = arMode === "marker";
    if (placeBtn) placeBtn.style.display = inMarker ? "none" : "";
    if (centerBtn) centerBtn.style.display = inMarker ? "none" : "";
    if (markerHint) markerHint.style.display = inMarker ? "flex" : "none";
  }

  // Cambia de modo EN CALIENTE (sin salir de AR). Detiene la fuente de vídeo
  // del modo anterior y arranca la del nuevo, reparentando el modelo.
  function switchMode(mode) {
    if (mode === arMode || !running) { syncModeUI(); return; }
    arMode = mode;
    syncModeUI();

    // Detener la fuente de vídeo actual.
    stopVideoSources();

    // Arrancar la del nuevo modo y, al estar lista, reparentar el modelo.
    if (arMode === "marker") {
      showToast("Apunta la cámara al marcador Hiro");
      initMarker(() => { applyModeToPlacedRoot(); });
    } else {
      initCamera(() => { applyModeToPlacedRoot(); placeModel(); });
    }
  }

  // Detiene y limpia SOLO las fuentes de vídeo (cámara directa y/o AR.js),
  // sin desmontar la escena ni el overlay. Se usa al cambiar de modo y al
  // salir de la sesión.
  function stopVideoSources() {
    // Cámara directa (modo cámara).
  ...
  function start() {
    ...
    try {
      ...
      cameraWatchdog = setTimeout(() => {
        const camReady = videoEl && videoEl.readyState >= 2;
        const mkReady = arToolkitSource && arToolkitSource.ready;
        if (!camReady && !mkReady) showToast("La cámara está tardando en responder. Revisa el permiso de cámara del sitio o ciérrala en otras apps.");
      }, CAMERA_WATCHDOG_MS);

      const onSourceReady = () => {
        running = true;
        applyModeToPlacedRoot();
        // En modo cámara el modelo aparece de una vez; en modo marcador
        // aparece cuando se detecta el Hiro.
        if (arMode === "camera") placeModel();
        rafId = requestAnimationFrame(renderLoop);
      };

      if (arMode === "marker") {
        showToast("Apunta la cámara al marcador Hiro");
        initMarker(onSourceReady);
      } else {
        initCamera(onSourceReady);
      }
    } catch (err) {
  ...
  function stop() {
    ...
    // Detiene ambas fuentes de vídeo (cámara directa y/o AR.js).
    stopVideoSources();

    // Devuelve el modelo a la escena (por si quedó bajo markerRoot).
    if (placedRoot && placedRoot.parent !== scene && scene) scene.add(placedRoot);
  ...
  // -----------------------------------------------------------------------
  // §7. LOOP DE RENDER — en modo cámara el modelo está fijo en la escena y
  // los gestos lo mueven. En modo marcador se actualiza el tracking de AR.js
  // cada frame (procesa el fotograma y dispara markerFound/markerLost, que
  // mueven markerRoot). En ambos casos se refrescan las etiquetas flotantes.
  // -----------------------------------------------------------------------
  function renderLoop() {
    rafId = requestAnimationFrame(renderLoop);
    if (arMode === "marker" && arToolkitSource && arToolkitSource.ready && arToolkitContext) {
  ...
  function toggleParamsSheet(force) {
    ...
    // Oculta la pista del marcador mientras el panel de parámetros está
    // abierto (para que no se solapen); al cerrar, syncModeUI la restaura si
    // seguimos en modo marcador.
    const markerHint = document.getElementById("arMarkerHint");
    if (markerHint) {
      if (open) markerHint.style.display = "none";
      else if (arMode === "marker") markerHint.style.display = "flex";
    }
  ...
  function wireUI() {
    ...
    // Interruptor de modo AR: Cámara (sin marcador) ↔ Marcador Hiro.
    document.querySelectorAll("[data-ar-mode]").forEach((b) => {
      b.addEventListener("click", () => switchMode(b.dataset.arMode));
    });
