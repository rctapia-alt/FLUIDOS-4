...
const AR = (() => {
  ...
  function initARScene() {
    ...
    placedRoot.visible = false; // oculto hasta que se detecte el marcador Hiro
  ...
  function start() {
    ...
    const hint = document.getElementById("arHint");
    if (hint) { hint.textContent = "Apunta la cámara hacia el marcador Hiro"; hint.style.display = "block"; }
  ...
  function updateBillboards() {
    const panel = document.getElementById("arReadoutPanel");
    if (!panel) return;
    if (!labelsOn || !placedRoot || !placedRoot.visible || !markerVisible) { panel.style.display = "none"; return; }
  ...
  function wireUI() {
    ...
    // parámetros. Estos faltaban en esta compilación.
    wireARTransport();
  }
