...
const Engine = (() => {
  ...
  // -----------------------------------------------------------------------
  // §0. FACTOR DE SEPARACIÓN / FUERZA CENTRÍFUGA RELATIVA (RCF, "número de g")
  //
  //     RCF = ω² r / g            [adimensional — múltiplos de g]
  //
  //     Relaciona la aceleración centrífuga local (ω²r) con la de la
  //     gravedad. Es el parámetro estándar para cuantificar qué tan intensa
  //     es la centrifugación (McCabe, Smith & Harriott; Geankoplis). En
  //     función de las rpm, sustituyendo ω = 2πN/60 y r en cm:
  //         RCF = 1.118 × 10⁻⁵ · r(cm) · N²
  //     (la constante 1.118e-5 = (2π/60)²/(100·g) con g = 9.80665 m/s²)
  // -----------------------------------------------------------------------
  const G0 = 9.80665; // m/s² — gravedad estándar
  function factorSeparacionRCF({ omega, r, rpm = null, g = G0 }) {
    const w = omega != null ? omega : rpmToOmega(rpm);
    const rcf = (w * w * r) / g;         // ω² r / g
    const aCent = w * w * r;             // aceleración centrífuga local (m/s²)
    return { rcf, aCent, g };
  }
