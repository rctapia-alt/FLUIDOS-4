...
(function () {
  ...
  function computeDecanter() {
    ...
    // Factor de separación (RCF / número de g), evaluado en el radio de
    // pared, donde la aceleración centrífuga es máxima — es la referencia
    // estándar para reportar la intensidad de la centrifugación.
    const sep = Engine.factorSeparacionRCF({ omega, r: DEC_R_WALL });
    ...
    Scene3D.updateDecanter(zn, SCALE_R_DECANTER, omega);
    ...
    UI.renderReadouts([
      ...
      { label: "Factor de separación (RCF · en pared)", value: UI.fmt(sep.rcf, 0), unit: "× g", status: "good" },
    ...
    UI.setViewerHud([
      ...
      { label: "RCF", value: `${UI.fmt(sep.rcf, 0)} × g` },
  ...
  function init() {
    ...
    function setPlaying(v) {
      sim.playing = v;
      Scene3D.setSimPlaying(v); // congela/reanuda el giro visual del modelo (escritorio y AR)
    ...
    Scene3D.setSimPlaying(sim.playing); // arranca congelado, coherente con "en pausa"
