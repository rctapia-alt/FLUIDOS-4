...
const Scene3D = (() => {
  ...
  let simPlaying = false;    // refleja sim.playing de main.js (setSimPlaying lo actualiza).
                             // Cuando es false, el modelo se CONGELA: ni el rotor/impulsor
                             // giran ni las gotas/flujo circulan. Solo Play lo reanuda.
                             // Es independiente de "spinning" (botón girar de escritorio).

  function setFrameCallback(fn) { frameCallback = fn; }
  function setRenderPaused(v) { renderPaused = v; }
  function setSimPlaying(v) { simPlaying = v; }
  ...
  function buildBrushedTexture() {
    ...
    // Filtrado anisotrópico + mipmaps trilineales: el acabado cepillado deja de
    // verse pixelado/con moaré en ángulos oblicuos (clave en AR, donde se mira
    // el modelo desde posiciones bajas y rasantes).
    if (renderer && renderer.capabilities) {
      brushedRoughnessTex.anisotropy = renderer.capabilities.getMaxAnisotropy();
    }
    brushedRoughnessTex.minFilter = THREE.LinearMipmapLinearFilter;
    brushedRoughnessTex.magFilter = THREE.LinearFilter;
    brushedRoughnessTex.generateMipmaps = true;
  ...
  function buildDecanter() {
    ...
    function makeDrop(geo, mat, parent) {
      ...
      const bs = 0.65 + Math.random() * 0.85;
      drop.userData.baseScale = bs; // guardado para restaurar tras el destello de coalescencia
      drop.scale.setScalar(bs);
      parent.add(drop);
      return drop;
    }
    ...
    for (let i = 0; i < 30; i++) {
      const d = makeDrop(heavyDropGeo, heavyDropMat.clone(), rotor); // material propio: el destello es por-gota
    ...
    for (let i = 0; i < 26; i++) {
      const d = makeDrop(lightDropGeo, lightDropMat.clone(), rotor); // material propio: el destello es por-gota
      d.userData.r = 1.12 + Math.random() * 0.15; // nacen en el anillo pesado
      lightParticles.push(d);
    }

    // ── ALIMENTACIÓN — chorro de mezcla que cae por el tubo superior (en
    //    x≈0.85) hacia el seno del líquido y se dispersa. Cada gota lleva un
    //    color mezcla (ni ámbar ni cian puro) para leerse como "emulsión sin
    //    separar" que recién entra. Al tocar el líquido se recicla arriba.
    const feedGroup = new THREE.Group();
    g.add(feedGroup);
    const feedDrops = [];
    const feedGeo = new THREE.SphereGeometry(0.02, 8, 8);
    const feedMat = new THREE.MeshStandardMaterial({
      color: 0xC9A063, emissive: 0x8a6a30, emissiveIntensity: 0.3, roughness: 0.6,
      transparent: true, opacity: 0.9
    });
    const FEED_X = 0.85, FEED_TOP = H * 0.5 + 0.35, FEED_BOT = H * 0.32;
    for (let i = 0; i < 14; i++) {
      const drop = new THREE.Mesh(feedGeo, feedMat.clone()); // opacidad independiente por gota
      drop.userData = {
        t: Math.random(),                 // 0 arriba → 1 abajo
    ...
    // ── DESCARGAS — gotas que SALEN por cada compuerta, cerrando el balance
    //    de materia visualmente: pesada (ámbar) sube por rA hacia su tubo;
    //    ligera (cian) por rB hacia el suyo. Viven fuera del rotor (no giran
    //    con él: representan producto ya separado saliendo del equipo).
    const dischargeGroup = new THREE.Group();
    g.add(dischargeGroup);
    const makeDischarge = (color, emissive, count, kind) => {
      const geo = new THREE.SphereGeometry(0.017, 8, 8);
      const baseMat = new THREE.MeshStandardMaterial({ color, emissive, emissiveIntensity: 0.3, roughness: 0.5, transparent: true, opacity: 0.9 });
      const arr = [];
      for (let i = 0; i < count; i++) {
        const drop = new THREE.Mesh(geo, baseMat.clone()); // opacidad independiente por gota
        drop.userData = { t: Math.random(), speed: 0.7 + Math.random() * 0.5, kind };
        dischargeGroup.add(drop);
        arr.push(drop);
      }
      return arr;
    };
    const dischargeHeavy = makeDischarge(0xF4C97A, COLORS.heavy, 10, "heavy");
    const dischargeLight = makeDischarge(0xBDEEF7, COLORS.light, 10, "light");

    // ── VECTORES DE FUERZA CENTRÍFUGA — flechas radiales a media altura que
    //    apuntan hacia afuera; su longitud/opacidad crece con el radio (∝ ω²r),
    //    haciendo visible que la fuerza de separación es mayor cerca de la
    //    pared. Se escalan en vivo con la intensidad (rpm) en el loop.
    const forceGroup = new THREE.Group();
    g.add(forceGroup);
    const forceVectors = [];
    const N_VEC = 8;
    for (let i = 0; i < N_VEC; i++) {
      const ang = (i / N_VEC) * Math.PI * 2;
      const arrow = makeFlowArrow(
        new THREE.Vector3(0, 0, 0),
        new THREE.Vector3(Math.cos(ang), 0, Math.sin(ang)),
        0x8FB8C9 // cian-gris neutro: "campo de fuerza", NO se confunde con la alerta roja
      );
      arrow.userData = { ang };
    ...
    dynamic.decanter = {
      ...
      feedGroup, feedDrops, FEED_X, FEED_TOP, FEED_BOT,
      dischargeGroup, dischargeHeavy, dischargeLight,
      forceGroup, forceVectors,
      omega: 314, intensidad: 1,
  ...
  function updateDecanter(zn, scaleR, omega) {
    const d = dynamic.decanter;
    if (!d.rotor) return;
    const rA = zn.rA * scaleR, rB = zn.rB * scaleR;

    // Intensidad centrífuga normalizada (0..~1.4) a partir de ω, para que
    // TODO el movimiento (giro, migración, chorro de alimentación, descargas)
    // se acelere al subir las rpm: es la señal visual del RCF creciendo.
    // ω a 3000 rpm ≈ 314 rad/s se toma como referencia → factor ≈ 1.
    if (omega != null) d.omega = omega;
    const OMEGA_REF = 314; // rad/s (~3000 rpm)
    d.intensidad = Math.max(0.12, Math.min((d.omega || OMEGA_REF) / OMEGA_REF, 1.6));
  ...
  function animate() {
    ...
    // Desvanecimiento gradual de la estela del trazador (independiente de
    // spinning, pero también se detiene en pausa para que el modelo quede
    // totalmente estático)
    if (simPlaying && currentEquip === "bowl" && dynamic.bowl.trail) {
    ...
    // El movimiento visual del modelo (giro del rotor/impulsor, migración de
    // gotas, circulación del flujo) solo avanza si la simulación está
    // reproduciéndose Y el usuario tiene activado "girar". Al pausar, el
    // modelo queda congelado en su pose actual hasta que se toque Play.
    if (spinning && simPlaying) {
      if (currentEquip === "decanter" && dynamic.decanter.rotor) {
        const dDec = dynamic.decanter;
        const intf = dDec.intensidad || 1; // factor de intensidad centrífuga (∝ rpm)
        dDec.rotor.rotation.y += dt * 1.4 * (0.5 + intf * 0.7); // el rotor gira más rápido a más rpm
        ...
        // migra hacia ADENTRO. Al cruzar r_i "coalescen" con un DESTELLO y
        // renacen dispersas en su zona de origen. La velocidad crece con el
        // radio local (∝ ω²r ⇒ campo centrífugo) y con la intensidad (rpm).
        ...
        const migrate = (list, dir, rHomeMin, rHomeSpan) => {
          for (let i = 0; i < list.length; i++) {
            ...
            ud.ang += dt * ud.slip * (0.6 + intf * 0.6);
            // aceleración radial ∝ (0.4 + r): más rápida lejos del eje. ×intensidad
            ud.r += dir * dt * 0.055 * (0.4 + ud.r) * (0.5 + intf);

            // brillo de coalescencia: al acercarse a la interfase la gota
            // "crece" y destella justo antes de fundirse en ella
            const distToIface = Math.abs(ud.r - riNow);
            if (distToIface < 0.12) {
              const glow = 1 - distToIface / 0.12; // 0..1
              drop.material.emissiveIntensity = 0.25 + glow * 0.9;
              drop.scale.setScalar((ud.baseScale || 1) * (1 + glow * 0.8));
            } else {
              drop.material.emissiveIntensity = 0.25;
              drop.scale.setScalar(ud.baseScale || 1);
            }
        ...
        // ── ALIMENTACIÓN — las gotas de mezcla caen por el tubo y, al llegar
        //    al seno del líquido, se reciclan arriba (chorro continuo). Un
        //    leve giro (swirl) evita que caigan en línea recta perfecta.
        if (dDec.feedDrops) {
          for (let i = 0; i < dDec.feedDrops.length; i++) {
            const drop = dDec.feedDrops[i];
            const ud = drop.userData;
            ud.t += dt * ud.speed * (0.6 + intf * 0.5);
            if (ud.t > 1) ud.t -= 1;
            const y = dDec.FEED_TOP - (dDec.FEED_TOP - dDec.FEED_BOT) * ud.t;
            const spread = ud.t * 0.12; // se abre un poco al caer
            drop.position.set(
              dDec.FEED_X + Math.cos(ud.swirl + t * 2) * spread + ud.jitter,
              y,
              Math.sin(ud.swirl + t * 2) * spread
            );
            drop.material.opacity = 0.9 * (1 - ud.t * 0.3);
          }
        }

        // ── DESCARGAS — gotas de producto saliendo por cada compuerta hacia
        //    su tubo. Pesada por rA (más lejos del eje), ligera por rB.
        if (dDec.dischargeHeavy) {
          const animDisch = (arr) => {
            for (let i = 0; i < arr.length; i++) {
              ...
              if (ud.t > 1) ud.t -= 1;
              if (ud.kind === "heavy") {
                // sube desde la compuerta A y sale radialmente por su tubo (+x)
                const yTop = dDec.H * 0.5 + 0.02;
                if (ud.t < 0.5) {
                  const k = ud.t / 0.5;
                  drop.position.set(0.66, dDec.H * 0.3 + (yTop - dDec.H * 0.3) * k, 0);
                } else {
                  const k = (ud.t - 0.5) / 0.5;
                  drop.position.set(0.66 + k * 0.85, yTop, 0);
                }
              } else {
                // sale por la compuerta B hacia su tubo (-z)
                const yTop = dDec.H * 0.5 + 0.02;
                if (ud.t < 0.5) {
                  const k = ud.t / 0.5;
                  drop.position.set(0, dDec.H * 0.3 + (yTop - dDec.H * 0.3) * k, -0.28);
                } else {
                  const k = (ud.t - 0.5) / 0.5;
                  drop.position.set(0, yTop, -0.28 - k * 0.45);
                }
              }
              drop.material.opacity = 0.9 * (1 - Math.max(0, ud.t - 0.7) / 0.3);
            }
          };
          animDisch(dDec.dischargeHeavy);
          animDisch(dDec.dischargeLight);
        }

        // ── VECTORES DE FUERZA CENTRÍFUGA — anclados en el radio de interfase,
        //    a media altura; longitud y opacidad ∝ (radio · intensidad), para
        //    mostrar que la fuerza de separación crece con r y con las rpm.
        if (dDec.forceVectors) {
          const yMid = 0; // media altura del rotor
          for (let i = 0; i < dDec.forceVectors.length; i++) {
            const arrow = dDec.forceVectors[i];
            const ang = arrow.userData.ang;
            const rBase = riNow * 0.7;
            arrow.position.set(Math.cos(ang) * rBase, yMid, Math.sin(ang) * rBase);
            const len = (0.18 + rBase * 0.22) * (0.4 + intf); // crece con r e intensidad
            arrow.setLength(len, len * 0.35, len * 0.22);
            if (arrow.line) arrow.line.material.opacity = Math.min(0.85, 0.3 + intf * 0.4);
            if (arrow.cone) arrow.cone.material.opacity = Math.min(0.9, 0.4 + intf * 0.4);
            if (arrow.line) arrow.line.material.transparent = true;
            if (arrow.cone) arrow.cone.material.transparent = true;
          }
          dDec.forceGroup.rotation.y = dDec.rotor.rotation.y * 0.15; // deriva lenta, no gira con el rotor
        }
  ...
  return {
    ...
    updateDecanter,
    setFrameCallback, setRenderPaused, setSimPlaying,
    stepDecanter,
